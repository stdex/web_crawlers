...
class Aggregator(object):
    ...
    def start_process(self):
        ...
        legislators_dict = collections.OrderedDict()
        for obj_page in object_list:
            ...
            for k in range(4,len(tables)):
                ...
                for s in range(1,len(work_all_legislators)-1):
                    ...
                    if(name_1 != ''):
                        if( not (str(hash(name_1)) in list(legislators_dict.keys())) ):
                    ...
                    if(name_2 != ''):
                        if( not (str(hash(name_2)) in list(legislators_dict.keys())) ):
                    ...
                    if(name_3 != ''):
                        if( not (str(hash(name_3)) in list(legislators_dict.keys())) ):
        ...
        worksheet_2.set_column('C:C', 20)
        worksheet_2.set_column('D:ZZ', 20)

        worksheet_2.write(0, 0, 'Last_name', header_format)
        worksheet_2.write(0, 1, 'First_name', header_format)
        worksheet_2.write(0, 2, 'Party', header_format)

        for indx in range(0,len(object_list)):
            worksheet_2.write(0, indx+3, 'Bill_'+str(indx+1), header_format)
        ...
        for hash_id, legis in legislators_dict.items():
            ...
            if(len(legis.get('name').split('\u3000\u3000')) == 2):
                worksheet_2.write(row, 0, legis.get('name').split('\u3000\u3000')[0], main_format)
                worksheet_2.write(row, 1, legis.get('name').split('\u3000\u3000')[1], main_format)
            elif(len(legis.get('name').split('\u3000')) == 2):
                worksheet_2.write(row, 0, legis.get('name').split('\u3000')[0], main_format)
                worksheet_2.write(row, 1, legis.get('name').split('\u3000')[1], main_format)
            else:
                worksheet_2.merge_range(row, 0, row, 1, legis.get('name'), main_format)
            worksheet_2.write(row, 2, legis.get('party'), main_format)
            #uprint(legis.get('name').split('\u3000\u3000'))
